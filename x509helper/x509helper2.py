...
from X509HelperApplication import X509HelperApplication

__id__ = "$Id: x509helper2.py 63 2014-03-19 03:49:33Z user $"
...
def parse_args():
    ...
    parser.add_argument('-o', '--output-directory',required=True, dest='output_directory',metavar="output directory", type=str,
                        help='Directory where unique validated keys and certificates should be written to')
    parser.add_argument('-i', '--input-directory',required=True, dest='input_directory',metavar="input directory", type=str,
                        help='Directory containing files to examine for valid PEM/DER formatted keys/certificates')        
    parser.add_argument('-b', '--blacklist-file', dest='blacklist_file',metavar="blacklist file",default="", type=str,
                        help='Filename of blacklist file which should contain a list of blacklisted modulus\' in openssl -modulus format')          
    parser.add_argument('-v','--verbose',dest='verbosity',default=0,action="count",help="Verbosity level for output. Use up to 5 times for full debugging information")    
    parser.add_argument('-F', '--out-format', metavar="output format",dest='output_format', type=str,
